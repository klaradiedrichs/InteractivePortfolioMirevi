...
const Frame = ({children,name,color,img,portalImg, spherePos,show,...props}) => {
  ...
  const {scale} = useSpring({ scale: boxactive && !hovered ? 0.6 : 1})
  ...
  const { opacity: opacityPoster } = useSpring({ 
    opacity: boxactive || hovered ? 0.3 : 1,
    // immediate: boxactive
  });
  const { opacity: textFadeIn } = useSpring({ 
    opacity: boxactive && !hovered ? 1 : 0,
    delay: boxactive && !hovered ? 300 : 0
    // immediate: boxactive
  });
          ...
          onPointerOut={() => active === null && show && setBoxActive(false)}>
            ...
            <group>
              ...
              <animated.mesh ref={myMesh} scale={scale} position-z={0.03} position-y={0}>
                ...
                {hovered && (
                <animated.mesh scale={scaleWindow} position-z={0.05}>
                    <circleGeometry />
                    {/* Sicht in Portal OnHover übr Button */}
                    
                        <MeshPortalMaterial ref={portalMaterial} transparent blur={0.3}>
