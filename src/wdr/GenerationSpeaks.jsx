...
export default function Experience() {
    ...
    const handleIdleVideoStart = (videoSrc) => {
        ...
        setTimeout(() => {
            setIdleVideo(false);
            // Reload the video texture if it's already loaded
...
const VideoMaterial = ({ videoSrc, opacity }) => {
    const videoTexture = useVideoTexture(videoSrc, { muted: false });

    return (
        <animated.meshBasicMaterial map={videoTexture} opacity={opacity} transparent />
    );
};
